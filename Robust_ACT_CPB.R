...
Robust_ACT_Estimate(ACT_Target, ACT_Values_Measured, ACT_Measurement_Times, Patient_Weight,
                    Bolus_Given, Bolus_Times, Infusion_Rates_Given, Infusion_Times,
                    Infusion_Durations)

#### C - Function creation ####

Robust_ACT_Estimate <- function(ACT_Target, ACT_Values_Measured, ACT_Measurement_Times,
  ...
  label <- data.frame(
    x = c(max(Line_Plot$TimeLine) - (max(Line_Plot$TimeLine)/3),
          max(Line_Plot$TimeLine) - (max(Line_Plot$TimeLine)/3),
          max(Line_Plot$TimeLine) - (max(Line_Plot$TimeLine)/3)),
    ...
    label = c(if(LD_New == 0) {paste0("New loading dose = 0 UI") } else {
  ...
  return(ggplot() +
           ...
           geom_text(aes(x = Perf_Time_New, y = (minArrowy - 30),
